...
export class IntelligenceEngine {
    async handleQuery(text: string): Promise<ParsedIntent> {
        ...
        // FIXED: Add the missing intentType declaration
    ...
    private calculateConfidence(text: string, intentType: string): number {
        ...
        const confidenceBoostKeywords: Record<string, string[]> = {
        ...
        const keywords = confidenceBoostKeywords[intentType] || [];
        const matches = keywords.filter(keyword => text.includes(keyword)).length;
        
        // Boost confidence based on keyword matches
        confidence += matches * 0.12;
        
        // Additional confidence for specific patterns - FIXED REGEX
